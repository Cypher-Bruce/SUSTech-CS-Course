...
public class FileManipulation implements DataManipulation {
    ...
    public String findMovieById(int id) {
        String line;
        String[] splitArray;
        int movieIdIndex = 0, runTimeIndex = 4, countryIndex = 2, yearReleasedIndex = 3, titleIndex = 1;
        StringBuilder sb = new StringBuilder();

        try{
            BufferedReader movieReader = new BufferedReader(new FileReader("movies.txt"));
            BufferedReader countryReader = new BufferedReader(new FileReader("countries.txt"));
            movieReader.readLine();
            while ((line = movieReader.readLine()) != null) {
                splitArray = line.split(";");
                if (Integer.parseInt(splitArray[movieIdIndex]) == id) {
                    sb.append("runtime:" + splitArray[runTimeIndex]);
                    sb.append(System.lineSeparator());

                    String countryLine;
                    String[] countrySplitArray;
                    int countryCodeIndex = 0, countryNameIndex = 1;
                    countryReader.readLine();
                    while ((countryLine = countryReader.readLine()) != null) {
                        countrySplitArray = countryLine.split(";");
                        if (countrySplitArray[countryCodeIndex].equals(splitArray[countryIndex])) {
                            sb.append("country:" + countrySplitArray[countryNameIndex]);
                            sb.append(System.lineSeparator());
                            break;
                        }
                    }

                    sb.append("year released:" + splitArray[yearReleasedIndex]);
                    sb.append(System.lineSeparator());
                    sb.append("title:" + splitArray[titleIndex]);
                    sb.append(System.lineSeparator());
                    break;
                }
            }
