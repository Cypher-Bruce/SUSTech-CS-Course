...
class ProblemSolvingAgent:
    ...
    def GBFS(self, obstacles, start_pos, goal_pos):
        ...
        queue = [(self.huffman_distance(start_pos, goal_pos), start_pos)]
        while queue:
            ...
            for neighbour, _ in self.neighbours_of(obstacles, node):
                if neighbour not in parent:
                    ...
                    heapq.heappush(queue, (self.huffman_distance(neighbour, goal_pos), neighbour))
        ...
        while queue:
            ...
            for neighbour, moving_cost in self.neighbours_of(obstacles, node):
                ...
                if neighbour not in min_cost or new_cost < min_cost[neighbour]:
                    ...
                    parent[neighbour] = node
                    heapq.heappush(queue, (new_cost + self.huffman_distance(neighbour, goal_pos), neighbour))
    ...
    def huffman_distance(self, node1, node2, coefficient=1):
        ...
        """
        return coefficient * (abs(node1[0] - node2[0]) + abs(node1[1] - node2[1]))
